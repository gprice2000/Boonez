...
const app = express();
const db = mongodb.MongoClient.connect('mongodb+srv://mazzaresejv:B00nze2022@cluster0.awpng.mongodb.net/myFirstDatabase?retryWrites=true&w=majority');


app.use(bodyParser.urlencoded({ extended: false }));
app.use(express.static(path.join(__dirname + '/New_capstone/Boonez-landing_pages')));
...
app.post('/login', function(req, res) {
  db.then(function(dbc) {
        dbc.db("Boonez").collection("profiles").findOne({username: req.body.username}, function(err,result){
          if (err) {
...
app.get("/pages/landing/signup.html", function(req,res) {
  res.sendFile("/pages/landing/signup.html", {
    root: "./",
  });
});
app.get("/pages/landing/login.html", function(req,res) {
  res.sendFile("/pages/landing/login.html", {
    root: "./",
  });
});
app.get('/view-feedbacks',  function(req, res) {
    db.find({}).toArrary().then(
      function(profiles) {
